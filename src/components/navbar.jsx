...
export default function Navbar({ categories, cart }) {
  ...
  return (
    <nav className="gridNavBar relative grid w-full  bg-white align-middle ">
      <div className="order-1">
        <Link to={"/"}>
          <Logo
            styling=" absolute z-30  h-16 w-15 text-purple/70 text-center text-[30px] flex items-center left-5 top-0 self-center "
            onClick={() => {
              navigate("/");
            }}
          />
        ...
        <div className="widthWithCalcMinusLogo absolute right-0  flex h-[64px] ">
          ...
          <div className="  flex w-9/12 flex-row justify-end maxLg:w-full">
            <div className=" h-30 flex w-full flex-row justify-around  maxSm:justify-end maxSm:pr-4">
              <div className="relative flex  flex-row maxSm:hidden">
                <input
                  type="search"
                  className=" relative appearance-none self-center rounded-md bg-khaki/20 p-1 focus:outline-purple/60 maxLg:w-[400px] maxLg:px-20 maxSm:w-full"
                />
              ...
              <div className="flex w-3/12 flex-row justify-end">
                {isLoggedIn?.success ? (
                  <>
                    <div className=" w-4/1 group h-full pt-5 hover:border-px hover:border-dashed hover:border-black hover:bg-khaki/10 hover:px-px maxSm:w-6/12 maxSm:pr-2">
                      <span className="flex flex-row  sm:justify-end   maxSm:justify-start maxSm:text-lg">
                        <IconContext.Provider
                          value={{
                            
                            className:
                              "self-center ml-2 fill-purple  md:w-6 md:h-6 ",
                          }}
                        >
                          <AiOutlineUser />
                        </IconContext.Provider>
      ...
      <div className="order-2 ">Test</div>
    </nav>
  );
}
